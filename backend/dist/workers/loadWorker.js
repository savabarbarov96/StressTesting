...
const runId = worker_threads_1.workerData?.runId || 'unknown';
...
const runLoadTest = async (spec) => {
    ...
    try {
        ...
        const result = await new Promise((resolve, reject) => {
            autocannonInstance = (0, autocannon_1.default)(requestConfig, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
            ...
            autocannonInstance.on('error', (error) => {
            ...
            let errorCount = 0;
            let latencySum = 0;
            autocannonInstance.on('response', (client, statusCode, resBytes, responseTime) => {
                requestCount++;
